// Process management: the fixed-size process table, the scheduler, and
// the fork/exit/wait/sleep/wakeup/kill machinery built on top of it.
//
// Every process is one of six states (see proc.h): UNUSED, EMBRYO,
// RUNNABLE, RUNNING, SLEEPING, or ZOMBIE. ptable.lock protects the whole
// table, including these state transitions, so that the scheduler and
// any process manipulating another process's entry never race.
...
// The process table itself: a fixed-size array of NPROC process control
// blocks. poc has no dynamic process allocation - allocproc() below just
// linearly scans this array for a free (UNUSED) slot.
struct {
...
found:
  ...
  //
  // This builds a fake stack frame so that the very first swtch() into
  // this process (from scheduler(), see below) behaves exactly like
  // returning from an ordinary call to swtch(): swtch pops a return
  // address off the stack and returns to it, so the fake return address
  // planted here (forkret) is where execution resumes. forkret in turn
  // simply returns, and *that* return address (trapret, pushed right
...
{
  struct proc *p;
  // Symbol name is derived from the build/initcode path given to ld's
  // -b binary option.
