...
// Vendored from limine-bootloader/limine-protocol - the one change from
// upstream: <stdint.h> swapped for poc-os's own types.h. This toolchain's
// bare-metal x86_64-elf-gcc ships a <stdint.h> that's just an
// #include_next wrapper expecting a real libc's copy to chain to (there
// isn't one - no sysroot, no -nostdinc even), so it fails outright; every
// other vendored/ported source tree in this repo (musl, bash, coreutils,
// nano) hits the same class of freestanding-environment gap and papers
// over it with a small hand-written shim rather than fighting the
// toolchain, so this does the same.
#include "types.h"
